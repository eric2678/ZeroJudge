...
class ZeroJudgeSetup(object):
    ...
    def _pull_tags(self, tmpdir):
        ...
        self._exec('git --git-dir=' + tmpdir + '/.git --work-tree=' +
                   tmpdir + ' checkout ' + tags[int(tagindex)])
    ...
    def _pull_latestversion(self, tmpdir):
        ...
        self._exec('git --git-dir=' + tmpdir + '/.git --work-tree=' +
                   tmpdir + ' checkout -b branch_'+latest_tag_line + ' ' + latest_tag_line)
        open(tmpdir + '/WebContent/META-INF/Version.txt',
    ...
    def install(self, dbpass, dbuser='root', githost='github.com', appname='ZeroJudge', version="latestversion", clean=True, SSL=False):
        ...
        # # 從遠端 pull 回來
        gituri = "https://"+githost+"/jiangsir/" + appname
        ...
        self._exec2('git clone ' + gituri + '.git/ '+gitpath+' --branch master')
        self._exec('rm -rf '+basepath)
        base =basepath[:basepath.rfind('/')]
        self._exec('mv '+base+'/'+gitpath+' '+base+'/'+ appname)

        self._exec2('sudo python3 ' + basepath +
                   "/setup.py upgradeZeroJudge --dbuser '"+dbuser+"' --dbpass '"+dbpass+"' \
    ...
    def upgradeZeroJudge(self, dbpass, warname=None, dbuser='root', githost='github.com', version="latestversion", clean=True, SSL=False):
        ...
        servergituri = "https://"+githost+"/jiangsir/" + \
            servername + ".git " + servertmpdir
        os.system('git clone ' + servergituri)
        ...
        if version == "latestversion":
            ...
            # ZeroJudge_Server
            cmd = 'git --git-dir=' + servertmpdir + \
                '/.git --work-tree=' + servertmpdir + ' branch -a'
            print("cmd=" + cmd)
            branchs = subprocess.check_output(
                cmd.split()).decode('utf-8').rstrip().split('\n')
            count = 0
            ...
            branchname = branchs[int(index)].split(
                '/')[len(branchs[int(index)].split('/')) - 1]
            self._exec('git --git-dir=' + servertmpdir + '/.git --work-tree=' +
                       servertmpdir + ' checkout ' + branchname)
            cmd = 'git --git-dir=' + servertmpdir + \
                '/.git --work-tree=' + servertmpdir + ' show-branch -g'
            message = subprocess.check_output(cmd.split()).decode('utf-8')
            print('message= ' + message)
            open(servertmpdir + '/WebContent/META-INF/Version.txt',
                 mode='w', encoding='utf-8').write(message)
        ...
        self._exec('ant -f ' + servertmpdir + '/build.xml -Dappname=' +
                   servername + ' -DTOMCAT_HOME=/usr/share/' + tomcatN + '/')
        # self._exec("clear")

        while int(subprocess.call("mysql -u "+dbuser+" -p'"+dbpass+"' -e \"USE "+dbname+";\"", shell=True)) != 0:
            dbpass = input("輸入資料庫 "+dbuser+" 密碼：")

        self._exec2("python3 " + apptmpdir +
                   "/build.py build --warname '" + warname + "' --dbuser '" + dbuser + "' --dbpass '" + dbpass+"' --SSL " + str(SSL))
        self._exec2("python3 " + servertmpdir +
                   "/build.py build --warname '"+servername + "'")
